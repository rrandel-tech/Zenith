...
namespace Zenith {
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (m_SceneState == SceneState::Edit)
		{
			ImGui::ImageButton("##PLAYBUTTON", (ImTextureID)(m_PlayButtonTex->GetRendererID()), ImVec2(32, 32), ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), ImVec4(0.9f, 0.9f, 0.9f, 1.0f));
			{
				OnScenePlay();
			}
		}
		...
		{
			if (ImGui::ImageButton("##EDITBUTTON", (ImTextureID)(m_PlayButtonTex->GetRendererID()), ImVec2(32, 32), ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), ImVec4(1.0f, 1.0f, 1.0f, 0.2f)))
			{
				OnSceneStop();
			}
		}
		ImGui::SameLine();
		if (ImGui::ImageButton("##PLAYBUTTONAGAIN", (ImTextureID)(m_PlayButtonTex->GetRendererID()), ImVec2(32, 32), ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), ImVec4(1.0f, 1.0f, 1.0f, 0.6f)))
		{
			ZN_CORE_INFO("PLAY!");
		}
		ImGui::End();
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Open Scene..."))
				{
					...
					std::string filepath = app.OpenFile("Zenith Scene (*.zscene)\0*.zscene\0");
					if (!filepath.empty())
					{
				...
				{
					auto& app = Application::Get();
					std::string filepath = app.SaveFile("Zenith Scene (*.zscene)\0*.zscene\0");
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyPressed(ZN_KEY_LEFT_CONTROL))
		{
			switch (e.GetKeyCode())
			{
			case KeyCode::G:
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (e.GetMouseButton() == ZN_MOUSE_BUTTON_LEFT && !Input::IsKeyPressed(KeyCode::LeftAlt) && !ImGuizmo::IsOver() && m_SceneState != SceneState::Play)
		{
