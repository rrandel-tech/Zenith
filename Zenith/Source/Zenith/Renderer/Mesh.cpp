...
#define ZN_MESH_LOG(...)
		...
		: m_FilePath(filename)
	{
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						ZN_MESH_LOG("Found existing bone in map");
		...
		{
			ZN_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				ZN_MESH_LOG("  {0} (Index = {1})", aiMaterialName.data, i);
				aiString aiTexPath;
				uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
				ZN_MESH_LOG("    TextureCount = {0}", textureCount);
				...
				ZN_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				ZN_MESH_LOG("    ROUGHNESS = {0}", roughness);
				...
				if (hasAlbedoMap)
				{
					...
					ZN_MESH_LOG("    Albedo map path = {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					ZN_MESH_LOG("    Normal map path = {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					ZN_MESH_LOG("    Roughness map path = {0}", texturePath);
					auto texture = Texture2D::Create(texturePath);
					if (texture->Loaded())
					{
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						ZN_MESH_LOG("    Metalness map path = {0}", texturePath);
				...
				{
					ZN_MESH_LOG("    No metalness texture");
				...
				for (uint32_t i = 0; i < aiMaterial->mNumProperties; i++)
				{
					...
					ZN_MESH_LOG("Material Property:");
					ZN_MESH_LOG("  Name = {0}", prop->mKey.data);
					// ZN_MESH_LOG("  Type = {0}", prop->mType);
					// ZN_MESH_LOG("  Size = {0}", prop->mDataLength);
					float data = *(float*)prop->mData;
					ZN_MESH_LOG("  Value = {0}", data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						ZN_MESH_LOG("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						ZN_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						ZN_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						ZN_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						ZN_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						ZN_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						ZN_MESH_LOG("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						ZN_MESH_LOG("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						ZN_MESH_LOG("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						ZN_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						ZN_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						ZN_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						ZN_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
#endif
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// ZN_MESH_LOG("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
