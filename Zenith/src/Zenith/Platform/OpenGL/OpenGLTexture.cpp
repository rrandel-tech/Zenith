...
namespace Zenith {
	...
	static GLenum ZenithToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
			...
			case Zenith::TextureFormat::Float16: return GL_RGBA16F;
		}
		ZN_CORE_ASSERT(false, "Unknown texture format!");
		return 0;
	}
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, ZenithToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height, 0, ZenithToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		{
			ZN_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
		...
		else
		{
			...
			ZN_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([instance, srgb]() mutable
		{
			...
			else
			{
				...
				GLenum internalFormat = ZenithToOpenGLTextureFormat(instance->m_Format);
				GLenum format = srgb ? GL_SRGB8 : (instance->m_IsHDR ? GL_RGB : ZenithToOpenGLTextureFormat(instance->m_Format)); // HDR = GL_RGB for now
	...
	void OpenGLTexture2D::Unlock()
	{
		...
		Renderer::Submit([instance]() {
			glTextureSubImage2D(instance->m_RendererID, 0, 0, 0, instance->m_Width, instance->m_Height, ZenithToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, instance->m_ImageData.Data);
		});
	}
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([instance, levels]() mutable
		{
			...
			glTextureStorage2D(instance->m_RendererID, levels, ZenithToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height);
			glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
			glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		: m_FilePath(path)
	{
		...
		Renderer::Submit([instance, faceWidth, faceHeight, faces]() mutable
		{
			...
			auto format = ZenithToOpenGLTextureFormat(instance->m_Format);
