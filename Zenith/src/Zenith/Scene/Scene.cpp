...
namespace Zenith {
	...
	void OnScriptComponentConstruct(entt::registry& registry, entt::entity entity)
	{
		...
		ZN_CORE_ASSERT(scene->m_EntityIDMap.find(entityID) != scene->m_EntityIDMap.end());
	...
	void Scene::OnRenderRuntime(Timestep ts)
	{
		...
		ZN_CORE_ASSERT(cameraEntity, "Scene does not contain any cameras!");
...

		{
			...
			for (auto entity : view)
			{
				...
				if (e.HasComponent<RigidBody2DComponent>())
				{
					...
					ZN_CORE_ASSERT(rigidBody2D.RuntimeBody);
	...
	Entity Scene::CreateEntityWithID(UUID uuid, const std::string& name, bool runtimeMap)
	{
		...
		ZN_CORE_ASSERT(m_EntityIDMap.find(uuid) == m_EntityIDMap.end());
