...
namespace Zenith {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			ZN_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		if (!scene || !scene->HasMeshes())
			ZN_CORE_ERROR("Failed to load mesh file: {0}", filename);

		//double factor;
		//scene->mMetaData->Get("UnitScaleFactor", factor);
		//ZN_CORE_INFO("FBX Scene Scale: {0}", factor);

		m_IsAnimated = scene->mAnimations != nullptr;
		m_MeshShader = m_IsAnimated ? Renderer::GetShaderLibrary()->Get("ZenithPBR_Anim") : Renderer::GetShaderLibrary()->Get("ZenithPBR_Static");
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			ZN_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			ZN_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				ZN_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						ZN_CORE_TRACE("Found existing bone in map");
		...
		if (scene->HasMaterials())
		{
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				ZN_CORE_INFO("Material Name = {0}; Index = {1}", aiMaterialName.data, i);
				aiString aiTexPath;
				uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
				ZN_CORE_TRACE("  TextureCount = {0}", textureCount);

				aiColor3D aiColor;
				aiMaterial->Get(AI_MATKEY_COLOR_DIFFUSE, aiColor);
				ZN_CORE_TRACE("COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);

				if (aiMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &aiTexPath) == AI_SUCCESS)
				{
					// TODO: Temp - this should be handled by Zenith's filesystem
					...
					if (texture->Loaded())
					{
						...
						ZN_CORE_TRACE("  Texture Path = {0}", texturePath);
						mi->Set("u_AlbedoTexture", m_Textures[i]);
						mi->Set("u_AlbedoTexToggle", 1.0f);
					}
					else
					{
						ZN_CORE_ERROR("Could not load texture: {0}", texturePath);
				...
				else
				{
					...
					ZN_CORE_TRACE("Mesh has no albedo map");
				}
				...
				if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						ZN_CORE_TRACE("  Normal map path = {0}", texturePath);
				...
				{
					ZN_CORE_TRACE("Mesh has no normal map");
				}
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						ZN_CORE_TRACE("  Roughness map path = {0}", texturePath);
				...
				{
					ZN_CORE_TRACE("Mesh has no roughness texture");
				}
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						ZN_CORE_TRACE("  Metalness map path = {0}", texturePath);
				...
				{
					ZN_CORE_TRACE("Mesh has no metalness texture");
				}
				...
				for (uint32_t i = 0; i < aiMaterial->mNumProperties; i++)
				{
					...
					ZN_CORE_TRACE("Material Property:");
					ZN_CORE_TRACE("  Name = {0}", prop->mKey.data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						ZN_CORE_TRACE("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						ZN_CORE_TRACE("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						ZN_CORE_TRACE("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						ZN_CORE_TRACE("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						ZN_CORE_TRACE("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						ZN_CORE_TRACE("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						ZN_CORE_TRACE("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						ZN_CORE_TRACE("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						ZN_CORE_TRACE("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						ZN_CORE_TRACE("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						ZN_CORE_TRACE("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						ZN_CORE_TRACE("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						ZN_CORE_TRACE("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// ZN_CORE_TRACE("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
	...
	{
		ZN_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		ZN_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		ZN_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		ZN_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		ZN_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		ZN_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		ZN_CORE_TRACE("------------------------------------------------------");
		ZN_CORE_TRACE("Vertex Buffer Dump");
		ZN_CORE_TRACE("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				ZN_CORE_TRACE("Vertex: {0}", i);
				ZN_CORE_TRACE("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				ZN_CORE_TRACE("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				ZN_CORE_TRACE("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				ZN_CORE_TRACE("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				ZN_CORE_TRACE("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				ZN_CORE_TRACE("--");
			}
		}
		else
