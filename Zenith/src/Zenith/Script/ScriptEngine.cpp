...
namespace Zenith {
	...
	{
		ZN_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	static void InitMono()
	{
		...
		auto domain = mono_jit_init("Zenith");

		char* name = (char*)"ZenithRuntime";
	...
	void ScriptEngine::LoadZenithRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain((char*)"Zenith Runtime", nullptr);
			mono_domain_set(domain, false);
			
			cleanup = true;
		}

		s_CoreAssembly = LoadAssembly("Resources/Scripts/Zenith-ScriptCore.dll");
	...
	{
		LoadZenithRuntimeAssembly(path);
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadZenithRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		ZN_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		ZN_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					ZN_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					ZN_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		ZN_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		ZN_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	static FieldType GetZenithFieldType(MonoType* monoType)
	{
		...
		switch (type)
		{
			...
			case MONO_TYPE_VALUETYPE:
			{
				...
				if (strcmp(name, "Zenith.Vector2") == 0) return FieldType::Vec2;
				if (strcmp(name, "Zenith.Vector3") == 0) return FieldType::Vec3;
				if (strcmp(name, "Zenith.Vector4") == 0) return FieldType::Vec4;
			}
		}
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			ZN_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
		...
		// Retrieve public fields (TODO: cache these fields if the module is used more than once)
		{
			...
			while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
			{
				...
				FieldType hazelFieldType = GetZenithFieldType(fieldType);
	...
	void ScriptEngine::InstantiateEntityClass(Entity entity)
	{
		...
		EntityInstance& entityInstance = entityInstanceData.Instance;
		ZN_CORE_ASSERT(entityInstance.ScriptClass);
	...
	{
		ZN_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		ZN_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	static uint32_t GetFieldSize(FieldType type)
	{
		...
		ZN_CORE_ASSERT(false, "Unknown field type!");
		return 0;
	}
	...
	{
		ZN_CORE_ASSERT(m_EntityInstance->GetInstance());
